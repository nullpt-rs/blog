...
Most anti-bots rely on aggressive browser fingerprinting[^fingerprint-js], collecting a plathora of device signals[^bot-test], and quietly deciding whether you're “human enough”. They often break for those using Linux, smaller browsers like Ladybird[^ladybird-turnstile], or even privacy-focused ones like Brave. They push the web toward a monoculture where only certain OS/browser combos are considered legitimate.
...
This blog post doesn’t present anything particularly new. In fact, much of it revisits topics covered before on this site. My goal here is simply to invite discussion and hear different perspectives on the matter.
